...
export const steps: StepData[] = [
  {
    ...
    desc: "初中阶段不规则图形面积问题的通用策略",
    detail: "1. 转化思想：所有不规则图形面积问题，通常都可转换成可求面积的直角三角形和规则图形（如梯形、矩形）的和差关系。\n\n2. 代数建模：求面积最值，本质是建立函数模型。我们需要将几何信息同步后，设一个未知数，构建面积的代数式（通常是二次函数），即可通过函数性质求得结论。",
    tts: "同学们好！在正式动笔之前，我们先梳理一下这类‘不规则图形求面积最值’问题的通用解题思路。第一，我们要学会转化。初中几何里，任何奇怪的形状都可以通过添加辅助线，拆解成我们熟悉的直角三角形和规则四边形。第二，求最值其实就是考代数。我们要把几何图形里的线段关系，转化成关于未知数x的二次函数，这样求最小值就变成了一个简单的代数问题。接下来，我们就按这个思路来拆解这道题。"
  },
  {
    title: "第二步：分析已知条件",
    icon: <BookOpen className="w-5 h-5" />,
    ...
    tts: "首先，我们在图中标注出已知条件：角A和角C都是直角，AD的长度是AB的两倍，DE等于6。我们的目标是求出四边形ABED面积的最小值。"
  },
  {
    title: "第三步：作辅助线 AF，寻找突破口",
